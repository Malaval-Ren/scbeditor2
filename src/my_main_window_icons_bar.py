...
# from .my_configuration_window import MyConfigurationWindow
# from .my_preference_window import MyPreferenceWindow
from .my_about_window import MyAboutWindow
# from .my_configuration_data import MyConfigurationData
...
class MyMainWindowIconsBar:
    ...
    def __init__( self, c_main_class, w_main_windows, list_application_info, a_top_frame_of_main_window):
        ...
        self.dict_mw_widgets_keys_name = None
        self.a_mw_key_number = None
    ...
    def __mwib_open_box( self):
        ...
        # self.c_the_log.add_string_to_log( 'Do preference')
        # self.w_front_window = MyPreferenceWindow( self.c_main_window, self.a_list_application_info)
        # self.w_front_window.pw_create_preference_window()
        # self.w_front_window = None

    # ####################### __configuration_box ########################
    def __mwib_save_box( self):
        """ Button configuration of the main window """
        # self.c_the_log.add_string_to_log( 'Do configuration')
    ...
    def mwib_create_top_bar_icons( self, i_row_line):
        ...
        i_column += 1
        if self.s_platform == "Darwin":
            a_button_preference = Button( self.a_top_frame_of_main_window, width=85, height=85, image=self.c_the_icons.get_open_photo(), compound="c", command=None, relief=s_button_style, highlightbackground='light grey')
        else:
            a_button_preference = Button( self.a_top_frame_of_main_window, width=85, height=85, image=self.c_the_icons.get_open_photo(), compound="c", command=None, relief=s_button_style, background=constant.BACKGROUD_COLOR_UI)
